...
│   │   ├── run-ticker.py             ← Per-ticker analysis runner (fundamental-thesis + fundamentals-scanner + entry-analyzer + gex-profile-equity)
...
30. **Ticker Analysis Runner (`run-ticker.py`)** — One-shot runner for all per-ticker tools.
    - **Purpose**: run the four ticker-specific tools on the same list of tickers in one command. Mirrors `run-all.py` but for ticker-scoped (vs market-scoped) analysis.
