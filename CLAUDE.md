...
27. **GEX Profile (`gex-profile.py`)** — SPX Gamma Exposure profile (Perfiliev method)
    ...
    - **Level definitions (canonical, per user spec)** — what each level *means*, which drives how it's computed:
      - **GEX 1** — the heaviest flow-driven target (the magnet price flow gravitates to). Heaviest **gross** gamma near spot, either side.
      - **HVL** — the calculated point where market makers flip long↔short gamma; the point of volatility equilibrium. The BS gamma flip (`gamma_flip_bs`).
      - **Call Resistance / Gamma Wall** — where flow premium was **sold** heaviest; acts as a **ceiling** that creates slowdowns. Heaviest call gamma **at/above spot**.
      - **Put Support** — inverse: where flow premium was **bought** heaviest; a **support shelf** **floor**. Heaviest put gamma **at/below spot**.
    - **MenthorQ-style named-levels block** (third Discord post): standardized industry taxonomy.
      - **Near-term (≤ `NEAR_TERM_DTE` = 45 DTE)**: `Call Resistance` (max +call_gex strike **with strike ≥ spot** — a ceiling), `Put Support` (max |put_gex| strike **with strike ≤ spot** — a floor), `HVL` (Zero-Gamma Level). The side constraint was added 2026-06-03: without it the heaviest call/put gamma can land on the wrong side of spot and mislabel a level (e.g. "Put Support" printing *above* spot, which is not a support shelf). **Structural levels are computed on the near-dated window, NOT the full chain** — see the MenthorQ re-validation note below.
      - **0DTE only**: `Call Resistance 0DTE / Gamma Wall 0DTE`, `Put Support 0DTE`, `HVL 0DTE` — same metrics restricted to today's expiry.
      - **GEX 1-5**: top 5 **gross-gamma** walls (`|call_gex| + |put_gex|`) within **spot ± 2%**, ranked by gross magnitude. Each tagged CALL+ or PUT- by the *net* sign. **Ranking is gross, not net** — see re-validation note.
      - **1D Max / 1D Min** (`compute_expected_move`): IV-derived 1-day expected-move boundaries = `spot ± spot × ATM_IV × √(1/252)`, where `ATM_IV` is the mean call+put IV at the strike nearest spot for the nearest expiry. Added to match MenthorQ's map (previously excluded as out-of-scope; the IV column is now parsed from the CBOE chain).
      - **Regime block**: `spot − HVL` for both near-term and 0DTE, labeled LONG GAMMA (pin/dampen) when ≥ 0, SHORT GAMMA (vol expansion) when < 0. Surfaces 0DTE-vs-longer-dated regime divergence (common case where today's expiry is pinning while structural positioning is short-gamma).
    - **MenthorQ re-validation (2026-06-03)** — user compared output to MenthorQ's published SPX levels (GEX1 7600, resistance 7630/7700, put support 7550) and flagged two discrepancies. Root cause was **not the formula** (the standard `gamma×OI×100×spot²×0.01` / call-positive-put-negative convention is correct and matches SqueezeMetrics/SpotGamma):
      - **(a) Aggregation window** — the full chain let far-OTM round-strike LEAPS puts (e.g. 7000 with massive OI) hijack `Put Support` (returned 7000). Restricting structural levels to ≤45 DTE returns Put Support 7550 = MenthorQ exact, and removes 8100/8000 LEAPS magnets from GEX 1-5.
      - **(b) GEX 1-5 ranking metric** — ranking by **net** GEX wrongly demoted the dominant wall. Strike 7600 had +11.76B call / −7.78B put = only +3.98B **net** but ~19.5B **gross**; net-ranking crowned 7700 (net +5.18B, gross only 5.5B) as GEX1. Switched to gross-gamma ranking within spot ±2% → GEX1 = 7600, matching MenthorQ's "primary GEX flat at 7600." (The big put gamma at 7600 nets out most of its call gamma, which is exactly why net hides it and MenthorQ uses total gamma.)
    - **HVL + Spot on ALL bar panels**: both the nearest-expiry (left) and OpEx (right) panels draw the cyan spot line and yellow-dashed per-expiry HVL/ZGL, each with its own legend (previously only the OpEx panel had a legend).
    - **Level-map image (`render_levels_map`, third image)**: MenthorQ-style horizontal-line map on a price axis — Call Resistance / GEX 1-5 / HVL / Put Support, 0DTE variants, 1D Max/Min, and spot, color-coded (red resistance, green support, yellow HVL + GEX1, cyan magnets, orange 1D bounds, cyan-dotted spot) with vertical label de-collision. Replicates the user's TradingView overlay.
    ...
    - **HVL / Zero-Gamma Level — Perfiliev Black-Scholes re-priced gamma flip (`gamma_flip_bs`)**: the canonical SqueezeMetrics / Perfiliev method. Holds OI fixed and **re-prices each contract's gamma via Black-Scholes** (`_bs_gamma`, r=q=0, T = business-days/262 with 0DTE floored at 1/262) at 60 spot levels across `[0.8, 1.2]×spot`, sums the signed dealer profile `Σ(call_gamma·OI − put_gamma·OI)·100·S²·0.01`, and returns the zero-crossing strike (linearly interpolated). Below the flip dealers are net short gamma (vol-expansion regime); above, net long (pinning / mean-reversion). Used for the named-levels HVL, both bar-panel HVL annotations, and the ZGL series in the evolution chart. Bubble magnitude = peak |profile| across the spot grid.
    - **Why BS re-pricing, not cumulative static GEX (replaced 2026-06-03)?** The original ZGL accumulated *static* per-strike GEX low→high and took the cumsum zero-cross. But a far-OTM strike's snapshot gamma is **not** the gamma it would carry if spot were actually there, so far put OI (7000s) formed a fake negative block that shoved the flip far above spot (0DTE HVL printed 7900 vs a true ~spot flip). The `gamma-profile.py` comparison quantified the error at **180–325 pts (2.4–4.3%)**. BS re-pricing makes distant strikes correctly contribute ~0, landing 0DTE HVL ≈ spot and the structural flip ~1% below spot on a down tape. The legacy `zero_gamma_flip` (static cumsum) is retained **only** so `gamma-profile.py` can display the legacy-vs-BS delta; it is no longer on the production path.
    - **`gamma-profile.py` (companion diagnostic)**: faithful port of the canonical `gammaProfileCommandLine.py` (BS gamma flip + the three Perfiliev profile curves: all / ex-next / ex-next-monthly). Imports `gex-profile.py` and prints a side-by-side table — BS flip vs legacy static ZGL — for 0DTE / OpEx / structural windows, confirming the walls (GEX1, Call Resistance, Put Support) match exactly while HVL diverges. Run `python gamma-profile.py [SPX]`. Not in `run-all.py` (on-demand validation tool).
    - **Put-GEX Watch (added 2026-06-12)**: from the evolution table (today + next 10 expiries), picks the **top 3 future expiries by max-put-GEX magnitude** (`top_put_watch_dates`) and tracks them day-over-day.
      - **Today's expiry (0DTE) excluded** from candidates — it would occupy a slot every day and churn the list every run.
      - **Strike-aware selection**: expiries whose put-GEX magnitudes are within 10% of each other (greedy grouping vs the group's leader, walking down by magnitude) are treated as equivalent; within such a group the **lowest put-wall strike wins** (the deepest SPX level is the one to watch).
      - **Output**: text block (`build_put_watch_report`: per-date DTE + put-wall strike + magnitude + prev-run comparison) + **3 separate full-size GEX bar charts** (`render_put_watch_chart`: same aggregate/clip/BS-flip pipeline as the main panels, orange title `PUT WATCH #n`, dashed orange line at the max-put strike, orange figure border). Watch dates also drawn as dashed orange verticals on the evolution chart.
      - **Persistence + change flag**: watch state saved to `gex-put-watch.json` (`run_date` + per-date strike/mag); on every run `diff_put_watch` compares vs the previous run — a date dropping while **still in the future** fires `⚠ WATCH DATE CHANGED` (dealer repositioning), a date dropping because it **expired/is today** gets only an informational note (prevents daily alert fatigue), additions fire `⚠ ADDED`, and persisting dates report put-wall strike/magnitude movement.

      **Worst-case stress test (Put-GEX Watch)**: (1) fewer than 3 future expiries with put gamma → take 1-2, skip with stderr note when 0; (2) first run / no state file → no flag, just save; (3) corrupt `gex-put-watch.json` → treated as first run, overwritten at end; (4) watched date becomes today → drops out by design, informational note not loud flag; (5) same-day re-run → state overwritten identically, diff empty (idempotent); (6) put_mag tie/similarity → deterministic 10%-group + lowest-strike rule; (7) weekend gap → prev `run_date` shown in the comparison header so the baseline is explicit; (8) Discord rate-limit on the now-7-post run → existing send helpers swallow errors per post.
    ...
    **Worst-case stress test**:
    ...
    9. **BS gamma flip: no zero-crossing in `[0.8,1.2]×spot`** (one-sided positioning) — `gamma_flip_bs` returns `(nan, mag)`; the HVL line/annotation is skipped via the `not np.isnan(zgl)` guard, evolution series shows a gap. No crash.
    10. **BS gamma flip: missing IV or 0DTE T=0** — contracts with `iv ≤ 0` dropped before profiling; `add_dte` floors 0DTE at `1/262` so the BS gamma isn't zeroed out (else the nearest expiry would vanish from the flip). Empty call or put leg → `(nan, 0.0)`.
    11. **BS gamma flip: spurious edge crossing on short-dated expiries** (fixed 2026-06-03) — for short DTE, gamma is razor-peaked at spot and underflows to ~0 across the rest of `[0.8,1.2]×spot`, so float sign-flicker at the edges manufactures fake zero-crossings (the evolution-chart ZGL line broke at ~0.82×spot ≈ 6216 vs a ~7560 true flip). Taking `np.diff(np.sign(prof))[0]` (first crossing from the bottom) grabbed the fake one. Fix: (a) reject crossings whose profile magnitude is `< 2% × peak` on both sides, then (b) among survivors pick the crossing **nearest spot** (the real short→long dealer transition). Verified all 11 evolution expiries land within ~70 pts of spot; synthetic test with planted 6162/6213 edge crossings returns the near-spot flip.
